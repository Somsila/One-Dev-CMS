...
function setParserOptions(opts) {
  parserOptions = defu.defu(opts, parserOptions);
}
let highlightPlugin;
...
async function _getHighlightPlugin(options) {
  const key = ohash.hash(JSON.stringify(options || {}));
  if (!highlightPlugin || highlightPlugin.key !== key) {
    const langs = Array.from(/* @__PURE__ */ new Set(["bash", "html", "mdc", "vue", "yml", "scss", "ts", "ts", "typescript", ...options.langs || []]));
...
const componentsManifestTemplate = (manifest) => {
  return {
    ...
    getContents: ({ app, nuxt, options }) => {
      const componentsMap = app.components.filter((c) => !c.island && (nuxt.options.dev || options.manifest.components.includes(c.pascalName) || c.global)).reduce((map, c) => {
...
function generateCollectionTableDefinition(collection, opts = {}) {
  ...
  const sqlFields = sortedKeys.map((key) => {
    ...
    if (!sqlType)
      throw new Error(`Unsupported Zod type: ${underlyingType.constructor.name}`);
...
const module$1 = kit.defineNuxtModule({
  ...
  async setup(options, nuxt) {
    ...
    options._localDatabase.filename = pathe.isAbsolute(options._localDatabase.filename) ? options._localDatabase.filename : pathe.join(nuxt.options.rootDir, options._localDatabase.filename);
    await promises.mkdir(pathe.dirname(options._localDatabase.filename), { recursive: true }).catch(() => {
    });
    ...
    const dumpGeneratePromise = processCollectionItems(nuxt, manifest.collections, options).then((fest) => {
      manifest.checksum = fest.checksum;
      manifest.dump = fest.dump;
      manifest.components = fest.components;
      return kit.updateTemplates({
        filter: (template) => [
